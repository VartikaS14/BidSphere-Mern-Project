...
export const Table = ({ products, handleSellProduct, delProduct, isAdmin, isWon, handleDeleteProduct }) => {
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500">
        <thead className="text-xs text-gray-700 uppercase bg-gray-100">
          <tr>
            ...
            <th scope="col" className="px-6 py-3">Bid Amount (USD)</th>
          ...
          {products.map((product, index) => (
            ...
            <tr className="bg-white border-b hover:bg-gray-50" key={index}>
              ...
              <td className="px-6 py-4">
                <img className="w-10 h-10" src={`http://localhost:5000/${product?.image?.filePath}`} alt="Product" />
              </td>
              {!isWon && (
                <>
                  <td className="px-6 py-4">
                      ...
                      {product?.isSoldout ? (
                        ...
                        <button
                          className={`py-1 px-3 rounded-lg ${
                            product?.isverify ? "bg-green text-white" : "bg-gray-400 text-gray-700 cursor-not-allowed"
                          }`}
                  ...
                  <td className="px-6 py-4 text-center flex items-center gap-3 mt-1">
                    ...
                    <NavLink
                      to={isAdmin ? `/product/admin/update/${product._id}` : `/product/update/${product._id}`}
